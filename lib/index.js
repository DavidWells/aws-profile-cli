...
function replaceDefaultProfile(newDefaultProfileName) {
  ...
  const newCredsFile = fileContents
    .replace(defaultProfile.rawText, newDefault)
    // Trim all leading whitespaces from lines
    // .replace(/[^\S\r\n]/gm, '')
...
function getAccessKey(text) {
  return text.match(/^\s?aws_access_key_id\s?=\s?([a-zA-Z0-9\S]*)/m)
}
/* Returns array */
function getSecretAccessKey(text) {
  return text.match(/^\s?aws_secret_access_key\s?=\s?([a-zA-Z0-9\S]*)/m)
}
